...
function createHouse(tileX, tileY) {
  ...
  const grass = new Entity(basePosX + 260, basePosY + 400, {
    sprite: "grass",
  })
  ...
  const possession = createRandomPossession(basePosX, basePosY)

  const enemy = new Enemy(basePosX + 200, basePosY + 200, { possession })
}

function createRandomPossession(basePosX, basePosY) {
  if (Math.random() > 0.5) {
    return new Possession(basePosX + 382, basePosY + 250, {
      sprite: "pie",
      z: 30,
    })
  }
