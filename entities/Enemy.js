...
export default class Enemy extends Character {
  ...
  update(dt) {
    ...
    if (this.holding) {
      if (this.possession.startPos.distance(this.pos) < 60) {
        this.drop()
        this.possession.startPos = V(
          this.possession.pos.x,
          this.possession.pos.y
        )
      }
      if (!this.following)
        this.setDirection(this.possession.startPos.subtract(this.pos))
    }
  }
  ...
  followPlayerInRange() {
    ...
    if (
      this.following &&
      this.pos.distance(this.following.pos) < this.cooldownRange
    ) {
      this.setDirection(this.following.pos.subtract(this.pos))
    } else if (wouldFollow && detectedTarget) {
      if (detectedTarget !== this.possession) this.drop()
      ...
      this.pickupIntent = detectedTarget === this.possession
    } else {
      this.following = false
  ...
  detectsEntity(entity) {
    const directionMatches =
      this.sprite.scale.x > 0
        ? entity.pos.x - this.pos.x < 0
        : entity.pos.x - this.pos.x > 0
    return (
      directionMatches && this.pos.distance(entity.pos) < this.detectionRange
    )
  }
  ...
  detectsTarget() {
    ...
    if (
      ...
      this.detectsEntity(this.possession)
    )
  ...
  onCollision(entity, data) {
    ...
    if (this.pickupIntent && entity === this.possession) {
